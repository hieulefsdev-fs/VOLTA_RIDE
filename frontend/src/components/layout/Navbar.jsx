        ...
        <div className="hidden md:flex items-center gap-3">
          {isAuthenticated ? (
            <div className="flex items-center gap-3">
              <Link to="/my-orders" className="text-sm text-gray-300 hover:text-white">
                Đơn của tôi
              </Link>
            ...
            {!isAuthenticated && (
              <div className="pt-3 border-t border-gray-800 flex gap-3">
                <Link to="/login" onClick={() => setOpen(false)}
                  className="flex-1 text-center text-sm py-2.5 rounded-xl border border-cyan-500/30 text-gray-300">
                  Đăng nhập
                </Link>
                <Link to="/register" onClick={() => setOpen(false)}
                  className="flex-1 text-center text-sm py-2.5 rounded-xl bg-gradient-to-r from-green-400 to-cyan-400 text-gray-900 font-semibold">
                  Đăng ký
                </Link>
              </div>
            )}
